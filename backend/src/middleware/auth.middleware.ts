...
export const verifyToken: RequestHandler = async (req, res, next) => {
  try {
    ...
    const userId = typeof rawSub === 'string' ? parseInt(rawSub, 10) : (rawSub as number | undefined);
    if (!userId || Number.isNaN(userId)) return res.status(401).json({ error: 'Invalid token subject' });
    ...
    const roles = rolesFromPayload ?? (await prisma.userRole.findMany({ where: { userId }, include: { role: true } })).map((r: { role: { name: string } }) => r.role.name);
...
export const requireRole = (allowedRoles: string[]) => {
  return (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    const ok = Array.isArray(user.roles) && user.roles.some((r: string) => allowedRoles.includes(r));
