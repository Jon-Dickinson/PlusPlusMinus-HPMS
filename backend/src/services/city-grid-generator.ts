...
export class CityGridGenerator {
    ...
    switch (hierarchyLevel) {
      ...
      case 2:
        return BUILDINGS.filter(b =>
          HIERARCHY_PERMISSIONS.CITY_LEVEL.includes(b.category as any),
        );
      case 3:
        return BUILDINGS.filter(b =>
          HIERARCHY_PERMISSIONS.SUBURB_LEVEL.includes(b.category as any),
        );
      default:
        return BUILDINGS.filter(b =>
          HIERARCHY_PERMISSIONS.SUBURB_LEVEL.includes(b.category as any),
  ...
  private getBuildingCounts(hierarchyLevel: number): Record<string, number> {
    switch (hierarchyLevel) {
      case 1:
        return {
          residential: 8,
  ...
  private generateShuffledCellOrder(): number[] {
  ...
  private computeResourceTotals(gridState: number[][]): Record<string, number> {
  ...
  private calculateResourceRatio(supply = 0, demand = 0): number {
