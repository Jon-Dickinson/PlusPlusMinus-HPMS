...
export async function createBuilding(data: any) {
  ...
  if (!categoryId && data.categoryName) {
    ...
    if (!cat)
      cat = await prisma.buildingCategory.create({
        data: { name: data.categoryName, description: data.categoryDescription ?? null },
      });
    categoryId = cat.id;
  }
...
export async function placeBuildingOnCity(
  cityId: number,
  buildingId: number,
  gx?: number | null,
  gy?: number | null,
...
export async function updateCityBuilding(
  id: number,
  data: Partial<{ gx: number; gy: number; quantity: number }>,
) {
